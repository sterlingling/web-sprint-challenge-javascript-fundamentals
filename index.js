...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// the nested fucntion is within the scope needed to let the variable ' internal ' be called or seen
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(data){
    const displayNames = [];
    data.forEach(element => displayNames.push(`name: ${element.animal_name}, scientific: ${element.scientific_name}`));
    return displayNames;
  }
  ...
  */
  ...
  function lowerCaseNames(data){
    const newArray = data.map((element)=> element.animal_name.toLowerCase());
    return newArray;
  }
  ...
  */
  ...
  function lowPopulationAnimals(data){
    const newArray = data.filter((element)=> element.population < 5);
    return newArray;
  }
